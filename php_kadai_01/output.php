...
//***************************************************************************************
    ...
    $files3 = "./files/name.txt";        //ファイルPathをSET
...
//Name File AddWrite
//***************************************************************************************
  $fp3 = @fopen($files3, "a");              //ファイルを "a" でオープン
  flock($fp3, LOCK_EX);                     //ファイルロック(排他ロック)
  fputs($fp3, $in["name"]."\n");         //ファイルに一行追記 ⇒ 行最後に"\n"で改行
  flock($fp3, LOCK_UN);                     //ファイルロック解除
  fclose($fp3);                             //ファイルを閉じる
...
// Write 評価値を保存（TOTAL値:::COUNT値）
  ...
  //次のページへ                                     //処理終了
  header("Location: input.php");               //次のページへ
  exit();                                      //処理終了
?>
